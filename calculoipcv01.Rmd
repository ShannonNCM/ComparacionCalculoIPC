...
```{r}
#esto es solo para una prueba, luego lo borro 
read_xlsx('relativopy.xlsx') %>% 
  rename(indpy = 1) %>% 
  mutate(relativo_r = relr$relativo) %>% 
  mutate(religual = if_else(relativo == relativo_r, TRUE, FALSE)) %>% 
  mutate(relres = abs(relativo-relativo_r)) %>% 
  filter(religual == FALSE) %>% 
  select(indpy, relativo, relativo_r, religual, relres) %>% 
  filter(relres > 1e-15)
```
...
```{r}
#esto es solo para una prueba, luego lo borro 
read_xlsx('promponpy.xlsx') %>% 
  rename(indpy = 1) %>% 
  mutate(region = as.integer(region)) %>% 
  left_join(prompon_r, by = c('region', 'cod_prod')) #%>% 
  #mutate(promponigual = if_else(prom_pon.x == prom_pon.y, TRUE, FALSE)) %>% 
  #mutate(promponres = abs(prom_pon.x-prom_pon.y)) %>% 
  #filter(promponigual == FALSE) %>% 
  #select(indpy, prom_pon.x, prom_pon.y, promponigual, promponres) %>% 
  #mutate(prompon_r = prompon_r$prom_pon) #%>%  **esto no funciona porque no tiene el mismo tamaño :c
  #filter(promponres > 1e-15)
```
...
```{r}
#esto es solo para una prueba, luego lo borro 
read_xlsx('indprodpy.xlsx') #%>% 
  #rename(indpy = 1) %>% 
  #mutate(region = as.integer(region)) %>% 
  #full_join(prompon_r, by = c('region', 'cod_prod')) %>% 
  #mutate(promponigual = if_else(prom_pon.x == prom_pon.y, TRUE, FALSE)) %>% 
  #mutate(promponres = abs(prom_pon.x-prom_pon.y)) %>% 
  #filter(promponigual == FALSE) %>% 
  #select(indpy, prom_pon.x, prom_pon.y, promponigual, promponres) %>% 
