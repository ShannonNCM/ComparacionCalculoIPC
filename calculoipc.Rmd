...
EXEC [dbo].[sp_get_precios_recolectados_mes] 2024, 2
```
...
EXEC sp_get_indice_grupo 2024, 2
```

```{sql connection=con, output.var="ponderacionesant"}
...
#estas ultimas 4 condicionales son en caso de que no aparezca un precio/cantidad o el precio/cantidad que se reporta tenga valor 0
...
incid_reg <- ind_gen %>% 
  ...
  mutate(inc_prod = ((ind_prod_rep-100)*participacion)/100)
#me falta arreglar esto para que tome los indices de las regiones, pero aun no tengo la funcion que me permite bajar los datos del indice de region en el sistema
```
...
writeData(indices, "Cuadro13", incid_prod)
writeData(indices, "Cuadro14", incid_div)
#writeData(indices, "Cuadro15", incid_reg)

saveWorkbook(indices, "indiceprelim_022024.xlsx", overwrite = TRUE)
```
